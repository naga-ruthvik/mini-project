...
def test_step(
    ...
    loss_fn: torch.nn.Module,
    device: torch.device,
    slice_2d_majority_voting: bool = False,
...
def train(
    ...
    for epoch in tqdm(range(epochs)):
        ...
        val_loss, val_acc, y_true, y_pred = test_step(
        ...
        if (
            ...
            and not isinstance(warmup_scheduler, type)
            and scheduler is not None
        ):
            with warmup_scheduler.dampening():
                if (
                    warmup_scheduler.last_step + 1
                    >= warmup_scheduler.warmup_params[0]["warmup_period"]
                ):
                    if isinstance(
