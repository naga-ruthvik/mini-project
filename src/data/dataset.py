...
class ADNIDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        # Convert the image to a numpy array and remove empty dimensions (e.g. 4D to 3D)
        img = np.squeeze(img.get_fdata(dtype="float32"))
        # Get the class name and index
        class_name = self.dataframe.diagnosis.values[
            index
        ]  # expects the dataframe in format (filename, label)
        ...
        if self.num_slices < img.shape[dim]:
            ...
            if self.slicing_from_left:
                initial_slice = max(
                    0,
                    int(img.shape[dim] / 2 - self.num_slices / 2 - self.slicing_offset),
                )
                final_slice = min(
                    img.shape[dim],
                    int(img.shape[dim] / 2 - self.slicing_offset + self.num_slices / 2),
                )
            else:
                initial_slice = max(
                    0,
        ...
        if resize_shape is not None:
            transformed_mri = torch.empty(
                (self.num_slices, 3, resize_shape[0], resize_shape[1])
            )
        else:
            if dim == 0:
                transformed_mri = torch.empty(
                    (self.num_slices, 3, img.shape[1], img.shape[2])
                )
            elif dim == 1:
                transformed_mri = torch.empty(
                    (self.num_slices, 3, img.shape[0], img.shape[2])
                )
            else:
                transformed_mri = torch.empty(
                    (self.num_slices, 3, img.shape[0], img.shape[1])
                )
        # Convert the 3D image in tensor
        img = torch.from_numpy(img).unsqueeze(
            0
        )  # e.g. (193, 80, 193) for coronal plane -> (1, 193, 80, 193)
        ...
        for i in range(img.shape[dim]):
            ...
            single_slice = torch.stack(
                (single_slice, single_slice, single_slice), dim=0
            )
            # Apply the transformation
            transformed_mri[i, :, :, :] = self.transform(single_slice)
            if self.data_augmentation_slice is not None:
                transformed_mri[i, :, :, :] = self.data_augmentation_slice(
                    transformed_mri[i, :, :, :]
                )
