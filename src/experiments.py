...
def run_experiment(train_df, val_df, test_df, config, writer, device, fold, trial=None):
    ...
    transform, data_augmentation, data_augmentation_slice = get_transforms(
        config["data_augmentation"],
        config["slicing_plane"],
        config["data_augmentation_slice"],
    )
    print(
        f"Using {data_augmentation_slice} as data augmentation strategy on 2D slices\n"
    )
    ...
    if config["image_type"] == "2D":
        ...
        print(
            f"Using {config['backbone']} as backbone for 2D images with Majority Voting\n"
        )
        ...
        if config["network_3D"] == "Axial3D":
            ...
            print(
                f"Using {config['backbone']} as backbone for MajorityVoting3D on 3D images\n"
            )
            ...
            print(
                f"Using {config['backbone']} as backbone for TransformerConv3DV2 on 3D images\n"
            )
        else:
    ...
    if config["load_pretrained_model"]:
        ...
        try:
            ...
            model.to(device)
            print(
                f"Successfully loaded pretrained model from {config['pretrained_model_path']}\n"
            )
        except FileNotFoundError:
            print(f"Pretrained model not found at {config['pretrained_model_path']}")
        except RuntimeError:
            print(
                f"Pretrained model at {config['pretrained_model_path']} has different architecture"
            )
    ...
    if config["resume"] or config["skip_training"]:
        ...
        best_model_path = os.path.join(
            config["experiment_folder"], f"fold_{fold}", "torch_model", "best_model.pth"
        )
    ...
    print(
        f"Frozen first {num_layers_to_freeze} layers out of {num_layers} in the backbone\n"
    )
    ...
    if config["warmup_scheduler"] is not None:
        warmup_scheduler = get_warmup_schedul(
            config["warmup_scheduler"], optimizer, config["warmup_kwargs"]
        )
        print(
            f"Using {config['warmup_scheduler']} warmup scheduler with {config['warmup_kwargs']}\n"
        )
    ...
    if config["scheduler"] is not None:
        ...
        if warmup_scheduler is not None:
            ...
            if config["scheduler_kwargs"] is not None:
                config["scheduler_kwargs"]["T_max"] = (
                    config["num_epochs"] - warmup_period
                )

        if scheduler_class is not None:
            # Safe unpacking of scheduler_kwargs
            scheduler_kwargs = config.get("scheduler_kwargs") or {}
    ...
    if config["skip_training"]:
        ...
        best_model_path = os.path.join(
            writer.get_logdir(), "torch_model", "best_model.pth"
        )
    model.load_state_dict(torch.load(best_model_path, map_location=device))
    ...
    writer.add_scalars(
        main_tag="Test results", tag_scalar_dict=metrics, global_step=fold
    )
    writer.close()
